...
| **Uniformity 2 is degrees and matchings** | no $k$-sunflower $\iff$ matching number and maximum degree both $\le k-1$ | `coq/TwoUniform.v` |
...
extremal problem at $D = \nu = k-1$. The lower half of that is proved:
$f(2,k) \ge k(k-1)+1$ for every odd $k$, from two disjoint copies of
$K_k$ (`coq/CliqueLowerBound.v`). The same identification pins the
sharp threshold of the spread hypothesis at uniformity 2 at $r = k$,
against the $2k-1$ this development proves — see
`rust/src/chvatal_hanson.rs`.
...
- **Testing what the kernel cannot check.** Both errors this
  ...
  that must survive. Underneath them, `coqchk`
...
Expected: every audited theorem (67 of them, including `f_2_3_eq_7`,
