...
**§57 is the current handover** and the right place to start cold (§54 is the previous one).
...
| `g_three_at_most_27`, `f_3_3_at_most_28`, `iota_three_at_most_thirteen` | `coq/PureLink.v` | **`f(3,3) ≤ 28`**, unconditionally, where Erdős–Rado gives 49 and `Sharp.sharp_beats_erdos_rado_at_three` reaches 32 only from a hypothesis about uniformity 4. With `Intersecting.lower_bound_3_3_20` this brackets `21 ≤ f(3,3) ≤ 28`. **`f(3,3)` is not an unknown sunflower number** — Abbott and Gardner settled it in 1969 and the lower bound here is the exact value; see row `AbbottGardner1969` below and `docs/roadmap.md` §33.1. This row called it the first unknown one until that was found. The asymptotic content is subsumed by Spencer 1977 (`docs/roadmap.md` §20.3); the finite values are not |
...
| `threshold_27_is_attained`, `non_strict_threshold_fails_at_3_3_3` | `coq/TightThreshold.v` | **The size hypothesis of `SpreadYieldsDisjoint 3 3 3` is exactly tight.** `nine27` is a 3-uniform family of exactly `3^3 = 27` members on nine points — 9-regular, so it meets the counting ceiling `9·9/3` — satisfying Rao's absolute condition at `r = 3` (`nine27_rao_spread`, by both reflective checkers) with no three pairwise disjoint members (`nine27_no_three_disjoint`, through `SpreadThreshold.decide_three_disjoint`). The non-strict variant of the statement is therefore **false** at `(3,3,3)`. It says nothing about `r*(3,3)` itself — that needs a 28th member, which cannot live on nine points — but any proof of `r*(3,3) = 3` must now be tight at 27 |
| `two_cover_at_most_3r_plus_1`, `two_cover_sharp_at_three_is_tight` | `coq/TwoCoverSharp.v` | **`τ = 2` gives `3r + 1`, exactly.** An intersecting 3-uniform family satisfying Rao's condition at `r ≥ 3`, covered by two points neither of which covers alone, has at most `3r + 1` members, and `hm_family` attains it at `r = 3`. Sharpens `TwoCover.two_cover_bound`'s `max(4r, 3r+4)` — 13 to 10 at `r = 3`, 16 to 13 at `r = 4` — by two finer counts: a common point on one side pins the other side to one pair plus one triple, and no common point on either side caps the two sides at seven together, because two members with disjoint tails pin the opposite side to four triples that cannot all occur. With `τ = 1` at `r²` and Frankl's `τ = 3` value 10, now proved in the next row, `I(3,3) = 10` is a theorem (`TauThreeTen.i_three_three_is_ten`) |
| `tau_three_ten`, `tau_three_at_most_ten`, `frankl_tau_three`, `ten_is_attained`, `tau_three_at_most_nine_is_false`, `i_three_three_at_most_ten`, `i_three_three_is_ten` | `coq/TauThreeTen.v` | **Covering number three: ten, exactly — Frankl's value in the kernel.** A 3-uniform intersecting family of distinct members that no two points cover has at most 10 members, with no degree or pair cap: fix a member `M`, count the members meeting `M` in two points by the *cores* (points common to every edge) of the three tail graphs, and prove that three nonempty pairwise cross-intersecting simple graphs have at most 9 edges plus core points — a star with three leaves or two intersecting subgraphs of a 4-cycle force a common point that, with covering number 3, cannot exist. Sharpens `TauThree.tau_three_bound`'s 16 to the truth; `k53`, the 3-subsets of a 5-set, attains it, so `TauThreeAtMost 9` is false. `TwoCover.FranklTauThree` — the hypothesis every `_from_frankl` corollary carried — is discharged, and `I(3,3) = 10` (star 9, two-cover 10, `τ = 3` 10, `k53` under the caps) is a theorem. Four mutations killed |
...
| `two_cover_degree_sum`, `two_cover_of_thirty_two_has_a_star_of_sixteen` | `coq/Support.v` | **What is left of the `ι(4,11)` ladder, made precise.** If two points meet every member then every member lies in one of their two stars, so the degrees sum to at least `\|F\|` — pigeonhole, and the one step of `docs/roadmap.md` §37.5 that was being made in prose. At the ladder's `\|F\| = 32` it forces a star of 16; the ladder has decided every `deg(0)` cube from 15 up UNSAT and `PureLink.link_at_point_bounded` with `g_three_at_most_26` makes 27 and above impossible, so **covering number two is refuted for that rung** and only `τ ≥ 3` survives. That is why `deg(0) = 13` and `14` were the whole of the remaining question **when this row was written**; both have since come back UNSAT (`docs/roadmap.md` §39 and §43), so the rung is closed at `ι(4,11) ≤ 31` and what survives is the *second opinion* on cube 13 — `docs/ladder/iota4_11.deg13.cryptominisat5.tsv`, §52. `TwoCover.v` proves far sharper things about `τ ≤ 2`; this is not a competitor to them |
...
| `sharp_forces_iota_four_at_most_31`, `sharp_beats_erdos_rado_at_three` | `coq/Sharp.v` | **`f(3,3) ≤ 32`**, against Erdős–Rado's 49 and this development's `f(3,3) ≥ 21` — a bound from a hypothesis about uniformity 4. It was described here as a bound on the *first unknown sunflower number*; `f(3,3)` has been known exactly since Abbott–Gardner 1969 (§33.1), so what this is worth is the conditional route, not a new value. Read as hardness, that is why `iota(4,10)` resisted two independent searches. The "read as a target" half of this row said settling one uniformity would be worth a new sunflower number at the next one down; it would not, because the number it lands on is `f(3,3)` and that is Abbott–Gardner's |
...
for every theorem in the "Closed" table (789 of them). The current
...
|---|---|---|
| Independent re-check | `make coqchk` | An `Admitted` or a second `Axiom` **anywhere** in the 52 modules, not just among the audited names; reliance on type-in-type, unsafe fixpoints, or assumed positivity |
...
Current mutation results: 175 mutations, all matching the outcome
declared in `tools/mutations.toml` — 172 killed outright, two genuine
...
`make coqchk` re-verifies all 52 modules with Coq's separate kernel
...
**Session N+16 (`docs/roadmap.md` §56).** The `m = 3` row's lower end is
the question *is there a 28-member 3-uniform family with no three
pairwise disjoint members, every point in at most 9 and every pair in at
most 3?* `coq/TightThreshold.v` certifies a **27**-member one, 9-regular
on nine points (`nine27`), so the strict size hypothesis of
...
anyway). On ten points the monolith was undecided after an hour, but cubed on the sorted degree sequence — eleven cubes, the partitions of the deficiency 6 — every cube is INFEASIBLE under CP-SAT (`docs/ladder/rstar_3_3_10.tsv`, `docs/roadmap.md` §56.9): **no 28-member family lives on ten points**. Eleven points is closed the same way (§56.10): 139 cubes, the partitions of the deficiency 15; 118 are INFEASIBLE flat and the 21 that stalled at 7200 s are INFEASIBLE in every sub-cube that additionally fixes the pair-degree profile of a minimum-degree point (`docs/ladder/rstar_3_3_11.tsv`, `docs/ladder/rstar_3_3_11.sub.tsv`, audited by `tools/audit11.py`). So **no 28-member family lives on eleven points either**, under CP-SAT. From above (§56.11): a member-type relaxation of the same question, with an exact LP-duality certificate checked without a solver (`make support15`), shows that under Frankl's `I(3,3) <= 10` a 28-member family has at most **fifteen** points, and the 378 degree-profile cubes the relaxation allows at 13, 14 and 15 points are all INFEASIBLE under CP-SAT with the Frankl cut (`make audit-support`). And all 738 twelve-point cubes are INFEASIBLE with the Frankl cut (`docs/ladder/rstar_3_3_support12.tsv`). Frankl's `I(3,3) <= 10` is now a theorem (`coq/TauThreeTen.v`, §56.13), so the cut `B = 10` those sweeps used is kernel-justified. So, **conditionally on one thing only — every CP-SAT INFEASIBLE verdict in these ladders being correct (no proof logs) — `M <= 27` and `r*(3,3) = 3`** (§56.12). The row in the table stays `{3, 4}` until the verdicts are replayed with checked proofs; the kernel-cut twelve-point sweep (`B = 16`) is no longer needed for that and continues only as a curiosity. The pinned 23-member object below and §22.5's
