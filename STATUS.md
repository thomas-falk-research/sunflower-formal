...
| `two_cover_at_most_3r_plus_1`, `two_cover_sharp_at_three_is_tight` | `coq/TwoCoverSharp.v` | **`τ = 2` gives `3r + 1`, exactly.** An intersecting 3-uniform family satisfying Rao's condition at `r ≥ 3`, covered by two points neither of which covers alone, has at most `3r + 1` members, and `hm_family` attains it at `r = 3`. Sharpens `TwoCover.two_cover_bound`'s `max(4r, 3r+4)` — 13 to 10 at `r = 3`, 16 to 13 at `r = 4` — by two finer counts: a common point on one side pins the other side to one pair plus one triple, and no common point on either side caps the two sides at seven together, because two members with disjoint tails pin the opposite side to four triples that cannot all occur. With `τ = 1` at `r²` and Frankl's `τ = 3` value 10 (cited, not proved), `I(3,3) = 10` is now one citation from a theorem |
...
for every theorem in the "Closed" table (771 of them). The current
...
|---|---|---|
| Independent re-check | `make coqchk` | An `Admitted` or a second `Axiom` **anywhere** in the 51 modules, not just among the audited names; reliance on type-in-type, unsafe fixpoints, or assumed positivity |
...
Current mutation results: 171 mutations, all matching the outcome
declared in `tools/mutations.toml` — 168 killed outright, two genuine
...
`make coqchk` re-verifies all 51 modules with Coq's separate kernel
